package com.example.backdemo.pool;
	...
	public PoolServer() {
		...
		this.addObserver(new Observer() {
			public void update(Observable o, Object arg) {
				try {
					for (int i = 0; i < 5; i++) {
						Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/bpm_user", "root",
								"root");
			...
			new Thread(new Runnable() {
				public void run() {
					try {
						...
						conn.prepareStatement("select * from customer_info");
